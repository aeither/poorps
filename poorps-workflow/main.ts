...
// --- Pyth Logic ---
const fetchPythPrice = (sendRequester: HTTPSendRequester, config: { url: string }): PythResponse => {
...
const getPrice = (runtime: Runtime<Config>): string => {
	runtime.log(`fetching price from url ${runtime.config.priceUrl}`)
	...
	const pythResponse = httpCapability
		...
		)({ url: runtime.config.priceUrl }) // Pass priceUrl as url to the fetcher
		.result()
	...
	if (pythResponse.parsed && pythResponse.parsed.length > 0) {
		...
		runtime.log(`Price: ${priceData.price} (conf: ${priceData.conf}, expo: ${priceData.expo})`)
